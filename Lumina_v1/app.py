    ...
    if st.button("Generate Learning Path"):
        if not topic:
            ...
            with st.spinner("Analyzing Knowledge Graph & Scaling Math..."):
                ...
                full_response = ask_groq(master_prompt)
                ...
                try:
                    ...
                    with col_v:
                        with st.container(border=True):
                            ...
                            # FIXED: Replaced use_container_width with width='stretch' to stop the warning
                            st.image(diagram_url, caption=f"Visualization: {topic}", width='stretch')
    ...
    if st.button("Generate CBSE Answer"):
        if not q_exam:
            ...
            with st.spinner("Drafting textbook-accurate answer..."):
                ...
                full_response = ask_groq(exam_prompt)
    ...
    if st.button("Analyze & Debug"):
        if not user_thought:
            ...
            with st.spinner("Analyzing logical consistency..."):
                ...
                response = ask_groq(debug_prompt)
    ...
    if col1.button("Generate Flashcards"):
        if not rev_topic:
            ...
            with st.spinner("Creating Interactive Flashcards..."):
                ...
                raw_response = ask_groq(fc_prompt)
        ...
        if st.button("Enter Viva Room"):
            if topic_input:
                ...
                with st.spinner("Examiner is reviewing the concept..."):
                    ...
                    st.session_state.cti_dna = ask_groq(dna_p)
